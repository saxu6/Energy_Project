...
def generate_energy_usage():
    ...
    for i in range(0, 24, 2):
        if 0 <= i < 8:
            ...
            usage.append(np.random.uniform(0.2, 0.8))
        elif 18 <= i < 24:
            usage.append(np.random.uniform(0.8, 2.0))
    return usage
...
for day in range(1, num_days + 1):
    ...
    daily_csv_filename = f"{month}_4bed_energy_consumption_day_{day}.csv"
    df.to_csv(daily_csv_filename, index=False)

# day avg
df_all = pd.concat([pd.read_csv(f"{month}_4bed_energy_consumption_day_{day}.csv") for day in range(1, num_days + 1)])
...
plt.axhline(y=monthly_avg_energy, color='r', linestyle='--', label='Monthly Avg Energy')
plt.xlabel("Day")
plt.ylabel("Energy Consumption (kWh)")
plt.title("Daily Average Energy Consumption vs Monthly Average for 4 bed")
plt.legend()
plt.grid()
plt.show()
